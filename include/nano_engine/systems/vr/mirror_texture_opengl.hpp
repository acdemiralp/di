...
public:
  ...
  {
    vr::VRCompositor()->GetMirrorTextureGL(static_cast<vr::EVREye>(eye), &id_, &handle_);
  }
  ...
  {
    vr::VRCompositor()->ReleaseSharedGLTexture(id_, handle_);
  }
  ...
  void          lock  () const
  {
    vr::VRCompositor()->LockGLSharedTextureForAccess(handle_);
  }
  void          unlock() const
  {
    vr::VRCompositor()->UnlockGLSharedTextureForAccess(handle_);
  }
